  ...
  _getData(int id) async {
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        ...
        toDos0 = user_tasks.where((v) => v.status == "待办").toList();
        toDos1 = user_tasks.where((v) => v.status == "进行中").toList();
        toDos2 = user_tasks.where((v) => v.status == "完成").toList();
      });
    } else {
      print(response.reasonPhrase);
    }
...
//长按后显示菜单
  void onLongPress(
    ...
    showMenu(context: context, position: position, items: list)
        .then((value) async {
      int task_id = int.parse(tasks.task_id);
